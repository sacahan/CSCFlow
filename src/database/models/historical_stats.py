...
class HistoricalStats(Base):
    """歷史統計記錄表：用於儲存不同時間區間（每日、每週、每月）的統計數據"""
    ...
    stats_type = Column(String(10), nullable=False)  # daily, weekly, monthly
    ...
    # 統計期間
    start_date = Column(Date, nullable=False)  # 統計開始日期
    end_date = Column(Date, nullable=False)  # 統計結束日期
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            stats_type.in_(["daily", "weekly", "monthly"]), name="check_stats_type"
        ),
        # 唯一性約束
        UniqueConstraint(
            "zip_code",
