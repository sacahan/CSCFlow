...
export const WeatherPanel: React.FC<WeatherPanelProps> = ({ location, weather, rainChance }) => {
    const [isLoading, setIsLoading] = useState<boolean>(!weather || rainChance === undefined);
    const [locationValue, setLocationValue] = useState<string>(location || '');
    const [weatherValue, setWeatherValue] = useState<string>(weather || '');
    const [rainChanceValue, setRainChanceValue] = useState<number | undefined>(rainChance);

    useEffect(() => {
       console.log(`WeatherPanel: Loaded weather for ${location} - Weather: ${weather}, Rain Chance: ${rainChance}%`);
        setLocationValue(location);
        setWeatherValue(weather);
        setRainChanceValue(rainChance);
        setIsLoading(location === '' || weather === undefined || rainChance === undefined);
